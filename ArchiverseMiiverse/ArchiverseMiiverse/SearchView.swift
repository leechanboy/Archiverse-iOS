                ...
                if !needsArchiveUnlock {
                    EmptyArchiveView(
                        ...
                        message: archiveSettings.isArchiveDateFilterEnabled
                            ? "No post results match the current archive date filter."
                            : "The open-source Archiverse repo currently exposes post search very conservatively, so some queries may return empty results."
                    )
                }
            } else {
                LazyVStack(spacing: 12) {
