...
struct OpeningSplashView: View {
    ...
    var playsTheme: Bool = true
    var showsSkipHint: Bool = true
    var layoutMode: OpeningSplashLayoutMode = .standard
    ...
    @State private var starsAreAnimating = false
    ...
    private var openingBackground: some View {
        ZStack {
            ...
            if let openingReferenceImage = BundleImageLoader.uiImage(named: "miiverse-opening-reference", fileExtension: "jpg") {
                Image(uiImage: openingReferenceImage)
                    .resizable()
                    .scaledToFit()
                    .padding(.horizontal, 18)
                    .opacity(shouldReduceEffects ? 0.05 : 0.09)
            }

            OpeningStarsLayer(
                isAnimating: starsAreAnimating,
                usesReducedMotion: shouldReduceEffects
            )
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
        .ignoresSafeArea()
    ...
    private func footerLogo(metrics: OpeningSplashMetrics) -> some View {
        Group {
            if let logoImage = BundleImageLoader.uiImage(named: "archiverse-logo", fileExtension: "png") {
    ...
    private func runOpeningSequence() async {
        ...
        audioPlayer.play(allowPlayback: playsTheme && shouldPlayOpeningAudio)

        withAnimation(.spring(response: shouldReduceEffects ? 0.42 : 0.72, dampingFraction: 0.86)) {
            introOpacity = 1
            introScale = 1
        }
        ...
        if shouldReduceEffects {
            ...
            withAnimation(.easeInOut(duration: 1.8).repeatForever(autoreverses: true)) {
                starsAreAnimating = true
            }
        }

        withAnimation(.linear(duration: splashDuration * 0.86)) {
            loadingProgress = 1
        }
    ...
    private var shouldReduceEffects: Bool {
        reduceMotion || ProcessInfo.processInfo.isLowPowerModeEnabled || UIScreen.main.isCaptured
    }

    private var shouldPlayOpeningAudio: Bool {
        !UIScreen.main.isCaptured && !ProcessInfo.processInfo.isLowPowerModeEnabled
    }

    private var splashDuration: Double {
        shouldReduceEffects ? 2.4 : 4.8
    }
}

private struct OpeningSplashMetrics {
    let size: CGSize
    let layoutMode: OpeningSplashLayoutMode
    ...
    var footerWidth: CGFloat {
        switch layoutMode {
        case .standard:
            return min(availableWidth, 640)
        case .immersive:
            return min(availableWidth, isLandscape ? 920 : 760)
        }
    }

    var footerReservedHeight: CGFloat {
        if layoutMode == .immersive {
            return isLandscape ? 132 : 208
        }
    ...
    var sceneWidth: CGFloat {
        switch layoutMode {
        case .standard:
            return min(availableWidth, isLandscape ? 620 : 560)
        case .immersive:
            return min(availableWidth, isLandscape ? 1080 : 760)
        }
    }
    ...
    var sceneHeight: CGFloat {
        ...
        case .immersive:
            return max(280, min(maxHeight, isLandscape ? 520 : 620))
        }
    }
    ...
    var progressHeight: CGFloat {
        ...
        return availableWidth < 390 ? 12 : 14
    }

    var logoWidth: CGFloat {
        if layoutMode == .immersive {
            return isLandscape ? min(availableWidth * 0.18, 180) : min(availableWidth * 0.42, 220)
        }
        ...
        GeometryReader { proxy in
            ForEach(stars) { star in
                Image(systemName: "star.fill")
                    ...
                    .foregroundStyle(
                        MiiversePalette.greenLight.opacity(
                            usesReducedMotion ? 0.55 : (isAnimating ? 0.86 : 0.38)
                        )
                    )
                    .scaleEffect(usesReducedMotion ? 0.88 : (isAnimating ? 1.0 : 0.72))
                    .animation(
                        usesReducedMotion
                        ? .default
                        : (
...
private enum OpeningSplashFormatter {
    private static let mainTimeFormatter: DateFormatter = {
    ...
    private static let mainPeriodFormatter: DateFormatter = {
        let formatter = DateFormatter()
    ...
    private static let cityTimeFormatters: [String: DateFormatter] = {
        Dictionary(uniqueKeysWithValues: OpeningCityLayout.allCases.map { city in
            let formatter = DateFormatter()
            formatter.dateFormat = "hh:mm a"
            formatter.timeZone = TimeZone(identifier: city.timeZoneIdentifier)
            formatter.locale = Locale(identifier: "en_US_POSIX")
            return (city.timeZoneIdentifier, formatter)
        })
    }()

    static func cityTime(_ date: Date, timeZoneIdentifier: String) -> String {
        guard let formatter = cityTimeFormatters[timeZoneIdentifier] else {
            return mainTimeFormatter.string(from: date)
        }
        return formatter.string(from: date)
    }

    static func mainTime(_ date: Date) -> String {
        mainTimeFormatter.string(from: date)
    }

    static func mainPeriod(_ date: Date) -> String {
        mainPeriodFormatter.string(from: date)
    }
}
