...
struct RemoteImageView: View {
    ...
    var body: some View {
        ...
        .applyRemoteImageLayout(
            aspectRatio: resolvedAspectRatio,
            frameHeight: frameHeight,
            scaling: scaling
        )
        .background(Color.white.opacity(0.6))
        .applyMediaClipping(for: scaling)
    ...
    private var resolvedAspectRatio: CGFloat? {
        ...
        if let loadedImage {
            let height = max(loadedImage.size.height, 1)
            return loadedImage.size.width / height
        }
...
actor RemoteImagePipeline {
    ...
    init() {
        ...
        configuration.urlCache = ArchiveCacheRetentionPolicy.makeImageCache()
    ...
    func image(for url: URL, targetPixelSize: CGSize) async throws -> UIImage {
        ArchiveCacheRetentionPolicy.touchImageCache()
...
private extension View {
    ...
    @ViewBuilder
    func applyRemoteImageLayout(
        aspectRatio: CGFloat?,
        frameHeight: CGFloat?,
        scaling: RemoteImageScaling
    ) -> some View {
        switch scaling {
        case .fill:
            self
    ...
    @ViewBuilder
    func applyMediaClipping(for scaling: RemoteImageScaling) -> some View {
        switch scaling {
        case .fill:
            self.clipped()
        case .fit:
