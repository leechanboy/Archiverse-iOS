...
struct HomeView: View {
    ...
    private var filteredFeaturedPosts: [ArchiversePost] {
    ...
    private var filteredRecentPosts: [ArchiversePost] {
        ArchiveDateFilter.filter(
            posts: recentPosts,
            cutoffDate: archiveSettings.selectedArchiveDate
        )
            ...
            VStack(alignment: .leading, spacing: 14) {
                if let logoImage = BundleImageLoader.uiImage(named: "archiverse-logo", fileExtension: "png") {
                    Image(uiImage: logoImage)
                        .resizable()
                        .scaledToFit()
                        .frame(maxWidth: 280, maxHeight: 40, alignment: .leading)
                        .frame(maxWidth: .infinity, alignment: .leading)
                }

                if let welcomeImage = BundleImageLoader.uiImage(named: "welcome-image", fileExtension: "png") {
                    Image(uiImage: welcomeImage)
                        .resizable()
                        .aspectRatio(880.0 / 280.0, contentMode: .fit)
    ...
    private var featuredScroller: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            LazyHStack(spacing: 14) {
                ForEach(filteredFeaturedPosts.prefix(8)) { post in
                    MiiverseCard {
                        VStack(alignment: .leading, spacing: 10) {
                            PostMediaPreview(
                                post: post,
    ...
    private var recentPostsList: some View {
        LazyVStack(spacing: 12) {
            ForEach(filteredRecentPosts) { post in
                MiiverseCard {
    ...
    private var hasAnyContent: Bool {
        !filteredFeaturedPosts.isEmpty || !filteredRecentPosts.isEmpty || !communities.isEmpty
    }
