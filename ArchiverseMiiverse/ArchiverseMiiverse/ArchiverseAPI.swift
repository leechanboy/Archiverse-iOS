...
struct ArchiverseAPI {
    ...
    func fetchHomepageDrawings(page: Int = 1) async throws -> [ArchiversePost] {
    ...
    func fetchRandomPosts(page: Int = 1) async throws -> [ArchiversePost] {
        let posts: [ArchiversePost] = try await fetch(
            path: "posts",
            queryItems: [
    ...
    func fetchUserPosts(userID: String, sortMode: String = "recent", page: Int = 1, beforeDate: Date? = nil) async throws -> [ArchiversePost] {
        var queryItems = [
        ...
        if let beforeDate {
            queryItems.append(
                URLQueryItem(name: "before_datetime", value: ArchiveDateFilter.apiValue(from: beforeDate))
            )
        }
    ...
    func fetchCommunityPosts(titleID: String, gameID: String, sortMode: String = "popular", page: Int = 1, beforeDate: Date? = nil) async throws -> [ArchiversePost] {
        var queryItems = [
...
enum ArchiveCacheController {
    static func clearAllCaches() async {
        ArchiveCacheRetentionPolicy.clearAPICache()
        ArchiveCacheRetentionPolicy.clearImageCache()
        await ArchiverseResponseCache.shared.clear()
        await ArchiversePostCache.shared.clear()
        await RemoteImagePipeline.shared.clear()
    }
}
