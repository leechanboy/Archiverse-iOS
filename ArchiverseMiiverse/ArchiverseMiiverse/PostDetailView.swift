...
struct PostDetailView: View {
                ...
                LazyVStack(alignment: .leading, spacing: 18) {
                    ...
                    if replies.isEmpty {
                        EmptyArchiveView(
                            ...
                            message: "This post has no visible replies in the current response, or the archive request did not complete."
                        )
                    } else {
    ...
    private func loadDetails() async {
        ...
        do {
            async let latestPost = api.fetchPost(id: currentPostID)
            async let fetchedReplies = api.fetchReplies(postID: currentPostID)

            post = try await latestPost
            yeahStore.refreshIfLiked(post)
            replies = try await fetchedReplies.filter { !$0.doNotShow }
            translatedDetailText = nil
            translationErrorMessage = nil
            pendingInlineTranslationText = nil
            isTranslatingPostText = false
            ...
            if let apiError = error as? ArchiverseAPIError, apiError.requiresArchiveUnlock {
                ...
                errorMessage = "Cloudflare verification is required before this post can be refreshed."
            } else {
                errorMessage = error.localizedDescription
            }
        }
            ...
            ForEach(items) { item in
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    ShareSnapshotImageView(
                        ...
                        scaling: item.asset.prefersContainedScaling ? .fit : .fill,
